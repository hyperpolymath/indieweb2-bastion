...
* The GUI/PWA dev server defaults to port 8443 (TCP). If that port is occupied, it falls back to 8080.
  For QUIC/HTTP3 testing, you can enable Deno’s `--unstable` net APIs and bind to a UDP port in the well‑known range.
...
For full compatibility across all shells (Bash, Fish, Nu, Zsh, Powershell, etc.), see link:scripts/install_vectors.adoc[Installation Vectors].

**Quick Start (Oil Shell):**
[source,oil]
----
...
== 🏗️ Project Structure
/
├── bastion/          # Hardened ingress gateway configs
├── odns/             # Oblivious DNS proxy/resolver
├── graphql-dns/      # GraphQL DNS API
├── surrealdb/        # Provenance graph schema + exports
├── docs/             # Documentation
├── scripts/          # Automation scripts
├── infra/            # Podman/K8s manifests
